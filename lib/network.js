...
function _applyOverrides(config) {

  function applyOverrideValue(value, fn) {
    if (value) {
      fn(value);
    }
  }

  function applyEnvOverride(name, fn) {
    applyOverrideValue(process.env[name], fn);
  }


  _.forEach(config.overrides, function(override) {

    const flag = override.env && override.env.flag;
    if (flag && process.env[flag]) {

      applyEnvOverride(override.env.port, function(value) {
        console.info(`override ${override.name}: port = ${value}`)
        config.edgemicro.port = value;
      })

      if (override.headers) {

        applyOverrideValue(override.headers.retarget, function(value) {
          console.info(`override ${override.name}: retarget_header = ${value}`)
          config.edgemicro.retarget_header = value;
        })

      }

      applyOverrideValue(override.log_to_console, function(value) {
        console.info(`override ${override.name}: logging.to_console = ${!!value}`)
        config.edgemicro.logging.to_console = !!value;
      })

    }
  })

  delete config.overrides;  // Don't write to cache-config
