 ...
 *  localproxy: {name:, path:, target_url:}, env: environment for which products should be loaded }
...
function loadConfiguration(thisconfig, keys, callback) {
    ...
    _load(config, keys, function(err, proxies, products) {
        ...
        if (proxies && products) {
            ...
            const mergedConfig = _merge(config, _mapEdgeProxies(proxies), _mapEdgeProducts(thisconfig.environment, products));
...
const _load = function(config, keys, callback) {
    ...
    async.parallel([
        ...
        try {
            ...
            var arrLen = apiProxies.length;
            if (proxyPattern) {
                debug("proxyPattern: " + proxyPattern + " enabled");
                for (var i = 0; i < arrLen; i++) {
                    if (!matchWildcard(apiProxies[i].apiProxyName, proxyPattern)) {
                        debug("ignoring " + apiProxies[i].apiProxyName + " proxy");
                        delete apiProxies[i];
                    }
                }
            }
        ...
        try {
            ...
            console.warn('warning: jwk keys are not enabled');
        }
        //add jwk support
...
const _mapEdgeProducts = function(environment, products) {
    //const path_to_proxy = {};
    ...
    products.filter(x=>x.environments.includes(environment)).forEach(function(product) {
